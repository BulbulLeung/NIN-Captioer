...
function fitnessLevelForScore(score) {
  if (score >= 70) return "green";
  if (score >= 40) return "yellow";
  return "red";
}
function aggregateCategoryDetails(captions, classifications) {
...
function buildDetailSlices(details) {
  ...
  const cy = 100;
  const r = 88;
  return active.map((detail, index2) => {
    ...
    return {
      id: detail.detailId,
      label: detail.label,
      color: DETAIL_COLORS[index2 % DETAIL_COLORS.length],
      count: detail.count,
      percent,
      path
    };
...
function CategoryPieChart({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "category-pie-card", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("h4", { className: "category-pie-title", children: title }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "category-pie-body", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pie-chart-wrap category-pie-wrap", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "svg",
          {
            ...
            children: slices.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "100", cy: "100", r: "88", className: "pie-empty" }) : slices.map((slice) => /* @__PURE__ */ jsxRuntimeExports.jsx(
              "path",
              {
                d: slice.path,
                fill: slice.color,
        ...
        slices.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "pie-empty-label", children: "No data" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { className: "pie-legend category-pie-legend", children: slices.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("li", { className: "legend-empty", children: "No details detected" }) : slices.map((slice) => /* @__PURE__ */ jsxRuntimeExports.jsxs("li", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "legend-swatch", style: { background: slice.color } }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "legend-label", children: slice.label }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "legend-stats", children: [
          slice.count,
          /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "legend-pct", children: [
            "(",
...
`;
const DEFAULT_CAPTION_PRESET_ID = "flux-krea2-lora-nsfw";
function createDefaultCaptionPreset() {
  return {
...
function CaptionEditor({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "caption-editor", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "caption-panel", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "caption-header", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("label", { htmlFor: "caption-tr", children: "Translated Caption" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "select",
          {
            ...
            children: LANGUAGES.map((lang) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: lang.code, children: lang.label }, lang.code))
          }
        )
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
...
function cacheKey(settings, text, direction) {
...
async function fetchWithTimeout$1(url, init, userSignal, timeoutMs) {
  ...
  try {
    ...
    if (reason === "timeout") {
      throw new Error(
        `Translation timed out (${Math.round(timeoutMs / 1e3)}s). Check that Ollama / LM Studio is running and the model is loaded.`
      );
    }
    if (reason === "cancel") {
...
async function translateText(settings, text, direction, signal) {
  const trimmed = text.trim();
  if (!trimmed) return "";
...
async function listModels(settings) {
  if (settings.provider === "lmstudio") {
    const base2 = settings.lmStudioBaseUrl.replace(/\/$/, "");
    const res2 = await fetch(`${base2}/models`);
    if (!res2.ok) throw new Error(`Cannot reach LM Studio (${res2.status})`);
    const data2 = await res2.json();
    return (data2.data?.map((m) => m.id).filter((id) => Boolean(id)) ?? []).sort(
  ...
  const data = await res.json();
  return (data.models?.map((m) => m.name).filter((name) => Boolean(name)) ?? []).sort(
    (a, b) => a.localeCompare(b)
  );
}
...
function SettingsDialog({ open, settings, onClose, onSave, onAutoSave }) {
  ...
  const skipPromptSave = reactExports.useRef(false);
  const draftRef = reactExports.useRef(draft);
  draftRef.current = draft;
  const baseUrl = draft.provider === "lmstudio" ? draft.lmStudioBaseUrl : draft.ollamaBaseUrl;
  ...
  const updateActivePreset = (patch) => {
    setDraft((prev) => ({
      ...prev,
      captionPresets: prev.captionPresets.map(
        (p) => p.id === prev.activeCaptionPresetId ? { ...p, ...patch } : p
      )
    }));
  };
  const addPreset = () => {
...
function useBidirectionalTranslate({
  ...
  const trTimer = reactExports.useRef(null);
  const abortRef = reactExports.useRef(null);
  ...
  const scheduleEnglishToTarget = reactExports.useCallback(
    (text) => {
      if (!enabled) return;
      if (enTimer.current) clearTimeout(enTimer.current);
  ...
  const scheduleTargetToEnglish = reactExports.useCallback(
    (text) => {
      if (!enabled) return;
      if (trTimer.current) clearTimeout(trTimer.current);
  ...
  const prevLangKey = reactExports.useRef(null);
  const englishSnapshot = reactExports.useRef("");
  const setEnglishSnapshot = reactExports.useCallback((text) => {
...
async function fetchWithTimeout(url, init, userSignal, timeoutMs) {
  ...
  try {
    ...
    if (reason === "timeout") {
      throw new Error(
        `Caption timed out (${Math.round(timeoutMs / 1e3)}s). Check that the vision model is loaded.`
      );
    }
...
function extractFinalCaption(raw) {
  let text = raw.trim();
  ...
  const paragraphs = text.split(/\n\s*\n/).map((p) => p.replace(/\n/g, " ").trim()).filter((p) => p.length > 40 && !/^[-*#]/.test(p) && !/^(shot type|camera)/i.test(p));
...
function activePresetPrompt(settings) {
  const preset = settings.captionPresets.find((p) => p.id === settings.activeCaptionPresetId) ?? settings.captionPresets[0];
  return preset?.prompt?.trim() ?? "";
}
function captionModelName(settings) {
  return settings.model || "";
}
async function captionWithLmStudio(settings, fullPrompt, mimeType, base64, signal) {
...
async function captionWithOllama(settings, fullPrompt, base64, signal) {
  const model = captionModelName(settings);
  if (!model) throw new Error("Model is required in Settings");
...
function App() {
  ...
  const [english, setEnglish] = reactExports.useState("");
  const [savedEnglish, setSavedEnglish] = reactExports.useState("");
  ...
  const missingCount = reactExports.useMemo(
    () => images.filter((img) => !img.hasCaption).length,
    [images]
  );
  const selectedFileName = reactExports.useMemo(() => {
  ...
  const resolveUnsaved = (action) => {
    setUnsavedOpen(false);
    unsavedResolver.current?.(action);
    unsavedResolver.current = null;
  };
  const saveCurrent = reactExports.useCallback(async () => {
  ...
  const onEnglishChange = (value) => {
    setEnglish(value);
    setEnglishSnapshot(value);
    scheduleEnglishToTarget(value);
  };
  const onTranslatedChange = (value) => {
    setTranslated(value);
  ...
  const startAutoCaption = async () => {
    ...
    try {
      ...
      if (batchCancelRef.current) {
        ...
        setStatus(
          failed > 0 ? `Auto Caption finished: ${done} ok, ${failed} failed` : `Auto Caption finished: ${done} image(s)`
        );
      }
    } finally {
      setBatchCaptioning(false);
      captionAbortRef.current = null;
      window.setTimeout(() => setStatus(""), 4e3);
    }
  };
  ...
  const clearEditor = reactExports.useCallback(() => {
    cancelInFlight();
    setSelectedPath(null);
    setEnglish("");
    setSavedEnglish("");
  ...
  const confirmDeleteSelected = reactExports.useCallback(async () => {
    if (!selectedPath) {
      setDeleteOpen(false);
      return;
    }
    ...
    const neighbor = images[idx + 1]?.path ?? (idx > 0 ? images[idx - 1]?.path : void 0);
    setDeleteOpen(false);
    try {
